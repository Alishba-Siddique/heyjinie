...
const CategoryPage = () => {
  ...
  const { category } = params; // Get the slug from the URL
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (response.success) {
          const categories = response.data.find(
            (cat: any) =>
              cat.name.toLowerCase().replace(/\s+/g, '') === category
          );
          if (categories) {
            setCategoryId(categories._id);
            fetchBrandsData(categories._id);
            fetchData(categories._id);
          } else {
  ...
  const fetchBrandsData = async (categoryId: string) => {
    setLoading(true);
    ...
    try {
      ...
      if (companiesResponse.success) {
        const filteredCompanies = companiesResponse.data.filter(
          (company: any) =>
            company.category_ids.some((cat: any) => cat._id === categoryId)
        );
    ...
    try {
      ...
      if (subcategoriesResponse.success) {
        const filteredSubCategories = subcategoriesResponse.data.filter(
          (sub: any) => sub.category_id._id === categoryId
        );
  ...
  const handleBrandClick = (
    ...
    router.push(`category/${slug}/${brandId}`);
  };
