...
export default function BrandShop() {
  ...
  const renderFeaturedProducts = () => {
    if (loading) {
      return <Skeleton className="mt-32 h-40 w-40 bg-[#d6dadb]" />;
    }
    if (featuredProducts.length === 0) {
      return <p className="font-xl">No Featured Products Available.</p>;
    }
    return (
      <>
            ...
            featuredProducts.map((product) => (
              ...
              >
                <figure className="at-giftimage">
                  <img
                    src={product.image_path || PLACEHOLDER_IMAGE}
                    alt={product.name || ''}
                  />
                </figure>
  ...
  const renderDealofDay = () => {
    ...
    if (products.length === 0) {
      return <h2 className="mt-7 font-xl">No Products Deals Available.</h2>;
    }
    return (
      <>
  ...
  const renderMenuProducts = () => {
    if (loading) {
      return <Skeleton className="mt-7 h-40 w-40 bg-[#d6dadb]" />;
    }
    if (products.length === 0) {
      return <h2 className="mt-7 font-xl">No Menu Products Available.</h2>;
    }
    return (
      <>
        {products.map((product) => (
