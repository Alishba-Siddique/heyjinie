...
//       ? slugify(decodeURIComponent(subcategorySlug as string))
//           // .replace(/-/g, ' ')
...
//             {unslugify(decodedSubcategoryName)}
//           </h1>
//         </div>
...
const PRODUCT_PLACEHOLDER = '/images/logoicons.png';
const BASE_URL = process.env.NEXT_PUBLIC_IMAGE_BASE_URL || ''; // This might not be needed if paths are absolute/full
const MODEL_BG_PATH = '/images/model-bg'; // Define locally for consistency

// Define type-based background settings (copied from pricefilter)
// --- Helper Function for Default Background Image (Modal Background) ---

const resolveImagePath = (path?: string | null): string => {
  ...
  return path.startsWith('/') ? path : `/${path}`; // Fallback for relative paths if BASE_URL is not set
};
...
const SubCategoryPage = () => {
  ...
  const handleProductClick = (product: Product) => {
    ...
    if (
      ...
      PFGalleryImages = product.product_background_images.filter(Boolean); // Assume relative paths
    } else if (
      typeof product.background_image === 'string' &&
      product.background_image.includes(',')
    ) {
      // Handle comma-separated string in product.background_image
      PFGalleryImages = product.background_image
        .split(',')
        .map((path) => path.trim())
        .filter(Boolean); // Assume relative
    } else if (
      typeof product.background_image === 'string' &&
      product.background_image.trim() !== ''
    ) {
      // Handle single image string in product.background_image
      PFGalleryImages = [product.background_image.trim()]; // Assume relative
    }
    PFGalleryImages = [...new Set(PFGalleryImages)];
    // Fallback to sticker_path or image_path if PFGalleryImages is still empty
    // if (PFGalleryImages.length === 0 && product.sticker_path)
    //   PFGalleryImages.push(product.sticker_path);
    // if (PFGalleryImages.length === 0 && product.image_path)
    //   PFGalleryImages.push(product.image_path);
    ...
    const productForModal: ProductForModal = {
      ...
      company_id: {
        ...
        name: companyData?.name || 'Brand Unavailable',
        company_logo: companyData?.company_logo // Assumed to be full/absolute path
          ? resolveImagePath(companyData.company_logo)
          : PRODUCT_PLACEHOLDER,
      },
      category_id: {
        _id: pageCategoryInfo?._id || 'cat-default-id',
      ...
      background_images: PFGalleryImages, // These are relative paths
      sticker_path: product.sticker_path ? product.sticker_path : undefined, // Relative path
      image_path: product.image_path ? product.image_path : undefined, // Relative path
      specification: product.specification || [
        // Default specification
        {
          name: 'Size',
  ...
  const handleAddToCart = () => {
    if (selectedProduct && cartContext) {
      const itemToAdd = {
        ...
        quantity,
        image_path:
          resolveImagePath(selectedProduct.background_images?.[0]) ||
          PRODUCT_PLACEHOLDER, // Use first background image
        sticker_path: selectedProduct.sticker_path,
        name: selectedProduct.name,
        description: selectedProduct.description,
        company_id: selectedProduct.company_id,
  ...
  const handlePrevImage = () => {
    if (
      ...
      return;
    const imageCount = selectedProduct.background_images.length;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <img
                        src={
                          resolveImagePath(displayImage) || PRODUCT_PLACEHOLDER
                        }
                        alt={product.name || 'Product Image'}
                        className="w-full h-full object-contain p-2"
                        onError={(e) => {
        ...
        {isFirstModalOpen &&
          ...
          (() => {
            const modalBackgroundStyle =
              getModalLeftPanelStyle(selectedProduct);
            const contentFirstImages = selectedProduct.background_images ?? [];
            const hasContentSlider = contentFirstImages.length > 1;
            let currentContentImageSrc = PRODUCT_PLACEHOLDER;
            ...
            if (
              contentFirstImages.length > 0 &&
              currentImageIndex < contentFirstImages.length &&
              contentFirstImages[currentImageIndex]
            ) {
              currentContentImageSrc = `${BASE_URL}/${contentFirstImages[currentImageIndex]}`;
              currentContentImageSrc = resolveImagePath(
                contentFirstImages[currentImageIndex]
              );
            } else if (selectedProduct.sticker_path) {
              // Fallback logic
                ...
                >
                  <div className="flex flex-col md:flex-row">
                    ...
                    >
                      ...
                      {hasContentSlider && contentFirstImages.length > 1 && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handlePrevImage();
                          }}
                          className="absolute left-2 top-1/2 transform -translate-y-1/2 z-10 bg-black bg-opacity-30 text-white p-2 rounded-full hover:bg-opacity-50"
                      ...
                      <figure className="at-productimg relative w-full h-full max-w-full flex items-center justify-center">
                        ...
                        {hasContentSlider && contentFirstImages.length > 1 && (
                          <button
                            ...
                            className="absolute right-2 top-1/2 transform -translate-y-1/2 z-10 bg-black bg-opacity-30 text-white p-2 rounded-full hover:bg-opacity-50"
                    ...
                    <div className="at-popupcontentside w-full md:w-[60%] p-6 flex flex-col justify-between">
                      <div>
                        ...
                        {selectedProduct.specification &&
                          selectedProduct.specification.length > 0 &&
                          selectedProduct.specification[0].values?.length >
                            0 && (
                            <div className="at-productsize mb-4">
                              <label className="block text-base font-medium text-gray-700 mb-1">
                                {selectedProduct.specification[0].name ||
        ...
        {isSecondModalOpen &&
          ...
          (() => {
            ...
            if (
              contentSecondImages.length > 0 &&
              currentImageIndex < contentSecondImages.length &&
              contentSecondImages[currentImageIndex]
            ) {
              currentContentImageSrc = `${BASE_URL}/${contentSecondImages[currentImageIndex]}`;
              currentContentImageSrc = resolveImagePath(
                contentSecondImages[currentImageIndex]
                  ...
                  <div className="flex justify-between items-center flex-col">
                    ...
                    <div className="at-modalcontent w-full">
                      ...
                      >
                        ...
                        {hasContentSlider && contentSecondImages.length > 1 && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handlePrevImage();
