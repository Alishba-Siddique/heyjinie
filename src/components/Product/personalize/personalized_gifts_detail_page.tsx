// src/components/personalized_gifts_detail_page/personalized_gifts_detail_page.tsx
...
const PersonalizedGiftDetailPage = ({
  ...
  const openPersonalizedGiftsModal = (image: GiftImage) => {
  ...
  useEffect(() => {
    const loadSessionData = () => {
      try {
        ...
        const storedImages = sessionStorage.getItem('selectedGiftImages');
        const storedId = sessionStorage.getItem('selectedProductId');

        if (!storedImages || !storedId) {
      ...
      <div className="at-shopcategories">
        ...
        <div className="at-shopcategoriesgrid">
          ...
          {!loading &&
            images.length > 0 &&
            images.map((image) => (
              <div className="at-shopcategoriesgriditem" key={image.image_id}>
                <figure>
                  <img
                    src={image.image_path}
                    alt={`${categoryName} gift`}
                    className="w-full h-64 object-cover cursor-pointer"
                    onClick={() => openPersonalizedGiftsModal(image)}
                    onError={(e) => {
